...
export default function CareerCard({ companyInfo, pageContent }: Props) {
  return (
    <Link
      href={"https://" + companyInfo.company_url}
      target="_blank"
      ...
      <div className="relative flex h-full transform items-end border-2 border-black bg-white transition-transform group-hover:-translate-x-2 group-hover:-translate-y-2">
        <div className="p-4 !pt-0 transition-opacity group-hover:absolute group-hover:opacity-0 sm:p-6 lg:p-8">
          ...
          <h2 className="mt-4 text-xl font-medium sm:text-2xl">
            {companyInfo.designation} @ {companyInfo.company}
          </h2>
          <p className="mt-4 text-sm sm:text-base">
            {toDateFormat(companyInfo.start_date)} {pageContent.betweenText}{" "}
            {toDateFormat(companyInfo.end_date)}
          </p>
          <p className="mt-4 text-sm sm:text-base">{companyInfo.location}</p>
        </div>
        <div className="absolute p-4 opacity-0 transition-opacity group-hover:relative group-hover:opacity-100 sm:p-6 lg:p-8">
          ...
          <p className="mt-4 text-sm sm:text-base">{companyInfo.description}</p>
